...
**Tracker updated:** 2026-07-30

| Status | Meaning | Tasks | Share |
|---|---|---:|---:|
| COMPLETE | Every acceptance item in the task is implemented and verified | 10 | 18% |
| PARTIAL | Useful scope is implemented, but acceptance items remain | 39 | 70% |
...
|---:|---|---|---|
| 1 | 9.1 Question Dataset | Expand the current 120-question bank toward the retained 400+ MVP target and complete editorial/source/licensing review | Final content review gates pass |
| 2 | 8.2 Integration Tests | Add device-level practice, exam, timer, background/resume, and cold-start recovery coverage | Integration suite passes on supported platforms |
| 3 | 8.4 Manual Testing | Validate timer backgrounding, expiry, clock lock, restoration, and analytics on real devices | Checklist results and defects are recorded |
| 4 | Accessibility Audit | Complete TalkBack, VoiceOver, contrast, keyboard, and 200% text-scale checks | Device audit findings are resolved |
| 5 | Performance Profiling | Validate release targets with the final question bank | Startup, query, and memory targets pass |
| 6 | 11.1/11.2 Platform Release Setup | Configure signing and validate production builds for Android and iOS | Signed release candidates install and run |
| 7 | 12.4 Beta Testing | Run signed beta builds after content approval | Release-candidate defects are closed |
...
- Production-reviewed 400+ question bank
...
**Remaining:** Reconcile the remaining richer session/performance fields, constraints, and indexes with the documented full schema. Persistent starred questions are implemented.
**Priority:** P0 | **Effort:** L
...
**Remaining:** Split the implemented database/repository operations into feature DAOs and add focused query-boundary tests. Exam, analytics, starred, and settings operations are implemented.
...
**Status:** COMPLETE | **Owner:** Unassigned | **Target:** Delivered | **Updated:** 2026-07-30
**Evidence:** `readiness_calculator.dart`, `readiness_insights_repository.dart`, progress providers, and focused tests implement the per-active-category eligibility gate, normalized scoring inputs, Australian values caps, qualifying-mock progression, explanations, and recommendations.
**Remaining:** None.
**Priority:** P1 | **Effort:** L
...
**Status:** COMPLETE | **Owner:** Unassigned | **Target:** Delivered | **Updated:** 2026-07-30
**Evidence:** `exam_repository.dart`, exam domain/history models, presentation, and repository tests enforce exactly five Australian values questions, the overall pass mark plus 5/5 values rule, persisted outcomes, and failure explanations.
**Remaining:** None.
**Priority:** P1 | **Effort:** S
...
**Remaining:** Separate the implemented starring and archived-question safeguards into dedicated domain use cases and expand focused widget coverage.
**Priority:** P1 | **Effort:** M
...
**Remaining:** Richer exit confirmation/retry edge cases, full responsive QA, and remaining accessibility verification. Configurable category, difficulty, and question-count flows are implemented.
**Priority:** P1 | **Effort:** L
...
**Remaining:** Broader dedicated widget coverage and final archive-message/device QA. Category/difficulty filtering and direct removal are implemented.
**Priority:** P1 | **Effort:** M
...
**Remaining:** Theme preference, notification settings if retained, explicit terms-of-service decision, and dedicated Settings widget tests. Privacy/licence/source links and the optional external Buy Me a Coffee support link are implemented; no features are gated.
**Priority:** P1 | **Effort:** M
...
**Remaining:** Expand the current validated 120-question bank to the retained 400+ MVP target, then complete editorial accuracy, official-source/taxonomy, and final licensing review.
**Priority:** P0 | **Effort:** S

Set up JSON structure:
- Create assets/data/questions.json with 400+ questions
...
**Remaining:** Validate taxonomy against current official material. Bundled exam configuration is connected to mock-exam composition and scoring.
**Priority:** P0 | **Effort:** S
...
Verify data:
- Validate all 400+ questions parse correctly
...
**Remaining:** EXPLAIN QUERY PLAN audit, final 400+ bank measurements, slow-query optimization, and caching only where measurements justify it.
**Priority:** P2 | **Effort:** M
...
|---|---|---|
| 2026-07-30 | Audited the repository against the tracker. Marked the revised readiness calculator and dual-condition mock scoring complete; recorded the implemented support link, configurable practice flows, starred filtering/removal, and config-driven exam behavior; reprioritized remaining content, integration, device, accessibility, performance, signing, and beta work. | Source and test evidence reviewed; question bank contains 120 questions; worktree was clean before this documentation update. A fresh Flutter analysis/test run was attempted but stalled without output, so the last recorded green automated run remains the current verification baseline. |
